...
private:
	...
	*/
public:
	QList<Diff> diff_lineMode(QString text1, QString text2, clock_t deadline, bool viewMode = false);
...
public:
	...
	/**
		...
		@param diffs LinkedList of Diff objects.
		@param colorOld is the html color for changes in the old version.
		@param colorNew is the html color for changes in the new version.
		@param showPilcrows specifies if pilcrows should be inserted.
		@return HTML representation.
	*/
public:
	QString diff_prettyHtml(const QList<Diff>& diffs, QString colorOld = "A3FFA3", QString colorNew = "FF8566", bool showPilcrows = false);

	/**
		Convert a Diff list into a pretty HTML report.
		@param diffs LinkedList of Diff objects.
		@param oldDiff returns the first version in html string.
		@param newDiff returns the newer version as html string.
		@param colorOld is the html color for changes in the old version.
		@param colorNew is the html color for changes in the new version.
		@param changesVisible specifies if changed whitespaces should be replaced with printable chars.
		@param equalLineMode specifies to make any changes to the whitespaces. If no equal lines shold be shown this should be false.
	*/
	void diff_prettyHtmlSideBySide(const QList<Diff>& diffs, QString& oldDiff, QString& newDiff, QString colorOld = "A3FFA3", QString colorNew = "FF8566", bool changesVisible = false, bool equalLineMode = true);

	/**
		replace every character in the string with space but keep linebreaks.
		@param input string.
		@param linebreak is the string to insert for every linebreak.
		@param spaceChar is the string to insert for every whitespace.
		@return modified string.
	*/
private:
	QString diff_createHtmlReplaceText(QString text, QString linebreak, QString spaceChar);
