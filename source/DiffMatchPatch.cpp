...
QList<Diff> DiffMatchPatch::diff_main(const QString& text1, const QString& text2)
{
	return diff_main(text1, text2, true);
}

QList<Diff> DiffMatchPatch::diff_main(const QString& text1, const QString& text2, bool checklines)
{
...
QList<Diff> DiffMatchPatch::diff_main(const QString& text1, const QString& text2, bool checklines, clock_t deadline)
{
...
QList<Diff> DiffMatchPatch::diff_compute(QString text1, QString text2, bool checklines, clock_t deadline)
{
	QList<Diff> diffs;
...
QList<Diff> DiffMatchPatch::diff_lineMode(QString text1, QString text2, clock_t deadline, bool viewMode /*= false*/)
{
...
QList<Diff> DiffMatchPatch::diff_bisect(const QString& text1, const QString& text2, clock_t deadline)
{
...
QList<Diff> DiffMatchPatch::diff_bisectSplit(const QString& text1, const QString& text2, int x, int y, clock_t deadline)
{
...
QList<QVariant> DiffMatchPatch::diff_linesToChars(const QString& text1, const QString& text2)
{
...
QString DiffMatchPatch::diff_linesToCharsMunge(const QString& text, QStringList& lineArray, QMap<QString, int>& lineHash)
{
	int lineStart = 0;
	int lineEnd = -1;
	QString line;
...
void DiffMatchPatch::diff_charsToLines(QList<Diff>& diffs, const QStringList& lineArray)
{
...
int DiffMatchPatch::diff_commonPrefix(const QString& text1, const QString& text2)
{
...
int DiffMatchPatch::diff_commonSuffix(const QString& text1, const QString& text2)
{
	// Performance analysis: http://neil.fraser.name/news/2007/10/09/
	const int text1_length = text1.length();
	const int text2_length = text2.length();
...
int DiffMatchPatch::diff_commonOverlap(const QString& text1, const QString& text2)
{
	// Cache the text lengths to prevent multiple calls.
	const int text1_length = text1.length();
	const int text2_length = text2.length();
...
QStringList DiffMatchPatch::diff_halfMatch(const QString& text1, const QString& text2)
{
	if (Diff_Timeout <= 0)
	{
...
QStringList DiffMatchPatch::diff_halfMatchI(const QString& longtext, const QString& shorttext, int i)
{
...
void DiffMatchPatch::diff_cleanupSemantic(QList<Diff>& diffs)
{
	if (diffs.isEmpty())
	{
		return;
	...
	while (thisDiff != NULL)
	{
		...
		else
		{
			...
			// sides of it.
			if (!lastequality.isNull() && (lastequality.length() <= std::max(length_insertions1, length_deletions1))
...
void DiffMatchPatch::diff_cleanupSemanticLossless(QList<Diff>& diffs)
{
...
int DiffMatchPatch::diff_cleanupSemanticScore(const QString& one, const QString& two)
{
...
QRegExp DiffMatchPatch::BLANKLINEEND = QRegExp("\\n\\r?\\n$");
QRegExp DiffMatchPatch::BLANKLINESTART = QRegExp("^\\r?\\n\\r?\\n");


void DiffMatchPatch::diff_cleanupEfficiency(QList<Diff>& diffs)
{
	if (diffs.isEmpty())
	{
		return;
	...
	while (thisDiff != NULL)
	{
		...
		else
		{
			...
			*/
			if (!lastequality.isNull() && ((pre_ins && pre_del && post_ins && post_del)
...
void DiffMatchPatch::diff_cleanupMerge(QList<Diff>& diffs)
{
...
int DiffMatchPatch::diff_xIndex(const QList<Diff>& diffs, int loc)
{
	int chars1 = 0;
	int chars2 = 0;
	int last_chars1 = 0;
...
QString DiffMatchPatch::diff_prettyHtml(const QList<Diff>& diffs, QString colorOld /*= "A3FFA3"*/, QString colorNew /*= "FF8566"*/, bool showPilcrows /*= false*/)
{
	QString html;
	QString text;

	QString linebreak = QString("%1<br>").arg(showPilcrows ? "&para;" : "");
	...
	foreach (Diff aDiff, diffs)
	{
		...
		{
		case INSERT:
			html += QString("<ins style=\"background:#%1;\">").arg(colorOld) + text + QString("</ins>");
			break;

		case DELETE:
			html += QString("<del style=\"background:#%1;\">").arg(colorNew) + text + QString("</del>");
			break;
	...
	html.replace(QString("<ins style=\"background:#%1;\"></ins>").arg(colorOld), "");
	html.replace(QString("<del style=\"background:#%1;\"></del>").arg(colorNew), "");
	html.replace(linebreak + "</ins>", "</ins>" + linebreak).replace(linebreak + "</del>", "</del>" + linebreak);
	return html;
}

void DiffMatchPatch::diff_prettyHtmlSideBySide(const QList<Diff>& diffs, QString& oldDiff, QString& newDiff, QString colorOld /*= "A3FFA3"*/, QString colorNew /*= "FF8566"*/, bool changesVisible /*= false*/, bool equalLineMode /*= true*/)
{
	QString linebreak = "<br>"; //changesVisible ? "&para;<br>" : "<br>";
	QString spaceChar = changesVisible ? "·" : "‡";
	spaceChar = equalLineMode ? spaceChar : "‡";
	...
	foreach (Diff aDiff, diffs)
	{
		...
		{
		case INSERT:
			newDiff += QString("<ins style=\"background:#%1;\">").arg(colorOld) + text + QString("</ins>");
			oldDiff += QString("<ins style=\"background:#%1;\">").arg(colorOld) + diff_createHtmlReplaceText(text, linebreak, spaceChar) + QString("</ins>");
			break;

		case DELETE:
			oldDiff += QString("<del style=\"background:#%1;\">").arg(colorNew) + text + QString("</del>");
			newDiff += QString("<del style=\"background:#%1;\">").arg(colorNew) + diff_createHtmlReplaceText(text, linebreak, spaceChar) + QString("</del>");
			break;

		case EQUAL:
			oldDiff += QString("<span>") + text + QString("</span>");
			newDiff += QString("<span>") + text + QString("</span>");
			break;
		}
	}

	oldDiff.replace(linebreak + "</ins>", "</ins>" + linebreak).replace(linebreak + "</del>", "</del>" + linebreak);
	oldDiff.replace(QString("<ins style=\"background:#%1;\"></ins>").arg(colorOld), "");
	oldDiff.replace(QString("<del style=\"background:#%1;\"></del>").arg(colorNew), "");
	newDiff.replace(linebreak + "</ins>", "</ins>" + linebreak).replace(linebreak + "</del>", "</del>" + linebreak);
	newDiff.replace(QString("<ins style=\"background:#%1;\"></ins>").arg(colorOld), "");
	newDiff.replace(QString("<del style=\"background:#%1;\"></del>").arg(colorNew), "");

	oldDiff.replace("&para;<br>", "&nbsp;&para;<br>").replace("‡", "");
	newDiff.replace("&para;<br>", "&nbsp;&para;<br>").replace("‡", "");
}

QString DiffMatchPatch::diff_createHtmlReplaceText(QString text, QString linebreak, QString spaceChar)
{
	int pos = 0;
	int lastPos = 0;

	text.replace("&nbsp;", " ");
	QString outputString = text;

	while (text.indexOf(linebreak, lastPos) >= 0)
	{
		pos = text.indexOf(linebreak, pos);

		outputString.replace(lastPos, pos - lastPos, spaceChar.repeated(pos - lastPos));
		lastPos = pos + linebreak.length();
		pos++;
	}

	if (lastPos != outputString.length())
	{
		outputString.replace(lastPos, outputString.length() - lastPos, spaceChar.repeated(outputString.length() - lastPos));
	}
...
QString DiffMatchPatch::diff_text1(const QList<Diff>& diffs)
{
	QString text;
...
QString DiffMatchPatch::diff_text2(const QList<Diff>& diffs)
{
	QString text;
...
int DiffMatchPatch::diff_levenshtein(const QList<Diff>& diffs)
{
	int levenshtein = 0;
	int insertions = 0;
	int deletions = 0;
...
QString DiffMatchPatch::diff_toDelta(const QList<Diff>& diffs)
{
	QString text;
...
QList<Diff> DiffMatchPatch::diff_fromDelta(const QString& text1, const QString& delta)
{
	QList<Diff> diffs;
...
int DiffMatchPatch::match_main(const QString& text, const QString& pattern,
							   int loc)
{
	// Check for null inputs.
...
int DiffMatchPatch::match_bitap(const QString& text, const QString& pattern, int loc)
{
...
double DiffMatchPatch::match_bitapScore(int e, int x, int loc, const QString& pattern)
{
...
QMap<QChar, int> DiffMatchPatch::match_alphabet(const QString& pattern)
{
	QMap<QChar, int> s;
	int i;
...
void DiffMatchPatch::patch_addContext(Patch& patch, const QString& text)
{
	if (text.isEmpty())
	{
		return;
	...
	while (text.indexOf(pattern) != text.lastIndexOf(pattern) && pattern.length() < Match_MaxBits - Patch_Margin - Patch_Margin)
	{
		...
		pattern = safeMid(text, std::max(0, patch.start2 - padding), std::min(text.length(), patch.start2 + patch.length1 + padding)
	...
	// Add the suffix.
	QString suffix = safeMid(text, patch.start2 + patch.length1, std::min(text.length(), patch.start2 + patch.length1 + padding)
...
QList<Patch> DiffMatchPatch::patch_make(const QString& text1, const QString& text2)
{
	// Check for null inputs.
	if (text1.isNull() || text2.isNull())
	{
...
QList<Patch> DiffMatchPatch::patch_make(const QList<Diff>& diffs)
{
...
QList<Patch> DiffMatchPatch::patch_make(const QString& text1, const QString& text2, const QList<Diff>& diffs)
{
...
QList<Patch> DiffMatchPatch::patch_make(const QString& text1, const QList<Diff>& diffs)
{
	// Check for null inputs.
	if (text1.isNull())
	{
...
QList<Patch> DiffMatchPatch::patch_deepCopy(QList<Patch>& patches)
{
...
QPair<QString, QVector<bool>> DiffMatchPatch::patch_apply(QList<Patch>& patches, const QString& sourceText)
{
...
QString DiffMatchPatch::patch_addPadding(QList<Patch>& patches)
{
...
void DiffMatchPatch::patch_splitMax(QList<Patch>& patches)
{
	...
	while (!bigpatch.isNull())
	{
		...
		while (!bigpatch.diffs.isEmpty())
		{
			...
			while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin)
			{
				...
				else if (diff_type == DELETE && patch.diffs.size() == 1 && patch.diffs.front().operation == EQUAL
...
QString DiffMatchPatch::patch_toText(const QList<Patch>& patches)
{
	QString text;
...
QList<Patch> DiffMatchPatch::patch_fromText(const QString& textline)
{
	QList<Patch> patches;
